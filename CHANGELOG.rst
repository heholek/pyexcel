...
#. implemented __len__ for pyexcel.Book and pyexcel.Sheet. Suddenly, Sheet could
   be a argument of Sheet. So Sheet(Sheet([[1, 2]])) works.
#. pyexcel.Sheet.group_rows_by_column(), helps categorize similiar fields into a
   book.
...
#. Sheet.column.column_name, Sheet.row.row_name is added to provide a short cut
   for Sheet.column['column name'], Sheet.row['row name']. In total, 4
   characters less in typing.
...
#. `custom_headers` for iget_records could have extra headers that does not
   ...
   may have done is to get a union of all headers in all excel sheets, then try
   get records from each sheet and merge all.
